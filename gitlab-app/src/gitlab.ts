...
export async function cancelOldPipelines(
  ...
  try {
    ...
    const pipelines = await response.json() as Array<{ id: number; status: string }>
    
    // Cancel older pending pipelines
    const older = pipelines.filter((p) => 
      p.id !== keepPipelineId && p.status === 'pending'
    )
    ...
    await Promise.all(
      older.map((p) =>
        fetch(`${BASE}/api/v4/projects/${projectId}/pipelines/${p.id}/cancel`, {
          method: 'POST',
          headers: { 'PRIVATE-TOKEN': TOKEN },
        }).catch((err) => console.warn(`Failed to cancel pipeline ${p.id}:`, err))
      )
    )
  } catch (error) {
