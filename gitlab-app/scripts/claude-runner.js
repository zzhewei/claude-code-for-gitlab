...
async function postComment(message) {
  const endpoint = context.resourceType === 'issue' 
  ...
  try {
    ...
    console.log(`✅ Posted comment to ${context.resourceType} #${context.resourceId}`);
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    const hasClaudeCode = process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_CODE_OAUTH_TOKEN;
    
    if (!hasClaudeCode) {
      throw new Error('No Claude authentication configured. Please set ANTHROPIC_API_KEY or CLAUDE_CODE_OAUTH_TOKEN.');
    }
...
Requested by @${context.author} in ${context.resourceType} #${context.resourceId}`;
      ...
      execSync(`git config user.email "claude-bot@${process.env.CI_SERVER_HOST}"`);
