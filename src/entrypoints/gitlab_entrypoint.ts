...
async function createMergeRequest(
  ...
  try {
    ...
    // Analyze changes to generate dynamic commit message
    ...
    let commitType = "feat";
    let commitScope = "";
    let commitDescription = "Apply Claude's suggestions";
    ...
    if (changedFiles.length > 0) {
      ...
      changedFiles.forEach(({ status, file }) => {
        ...
        if (status.includes("A")) modifications.add("add");
        if (status.includes("M")) modifications.add("modify");
        if (status.includes("D")) modifications.add("delete");
        if (status.includes("R")) modifications.add("rename");
      });

      // Set commit type based on modifications
      if (modifications.has("add") && modifications.size === 1) {
        commitType = "feat";
      } else if (modifications.has("delete") && modifications.size === 1) {
        commitType = "refactor";
      } else if (modifications.has("modify") || modifications.size > 1) {
        commitType = "fix";
      }

      // Set scope based on file types
      if (fileTypes.has("test")) {
        ...
        commitScope = Array.from(fileTypes)[0];
      }
      ...
      if (changedFiles.length === 1) {
        ...
        commitDescription = `update ${fileName}`;
      } else if (changedFiles.length <= 3) {
        const fileNames = changedFiles
          .map(({ file }) => file.split("/").pop()?.split(".")[0] || "file")
          .join(", ");
        commitDescription = `update ${fileNames}`;
      } else {
        commitDescription = `update ${changedFiles.length} files`;
      }
    }

    const commitMessage = `${commitType}${commitScope ? `(${commitScope})` : ""}: ${commitDescription}

${changedFiles
  .map(({ status, file }) => {
    const action = status.includes("A")
      ? "Add"
      : status.includes("M")
        ? "Modify"
        : status.includes("D")
...
This commit was automatically generated by Claude AI for ${process.env.CLAUDE_RESOURCE_TYPE} #${process.env.CLAUDE_RESOURCE_ID}.`;
    ...
    // Generate dynamic MR title based on changes
    let mrTitle = `${commitType}${commitScope ? `(${commitScope})` : ""}: ${commitDescription}`;

    // Add context for the original request
    const resourceContext = `${process.env.CLAUDE_RESOURCE_TYPE} #${process.env.CLAUDE_RESOURCE_ID}`;
    if (mrTitle.length + resourceContext.length + 3 < 255) {
      // GitLab title limit
      mrTitle += ` - ${resourceContext}`;
    }

    // Generate dynamic MR description based on changes
    ...
    // Generate meaningful description based on the type of changes
    let purposeDescription = "";
    let changesDetail = "";

    if (commitType === "feat") {
      purposeDescription = changedFiles.some((f) => f.file.includes("test"))
        ? "Add new test coverage"
        : "Implement new functionality";
    } else if (commitType === "fix") {
      purposeDescription = "Fix issues and improve code quality";
    } else if (commitType === "refactor") {
      purposeDescription = "Refactor and clean up codebase";
    }

    // Analyze changes to provide more context
    ...
    const hasDocs = changedFiles.some(
      (f) => f.file.includes("README") || f.file.includes("doc"),
    );
    ...
    const improvements = [];
    if (hasSource) improvements.push("code implementation");
    if (hasTests) improvements.push("test coverage");
    if (hasConfig) improvements.push("configuration");
    if (hasDocs) improvements.push("documentation");

    if (improvements.length > 0) {
      changesDetail = `Updates include: ${improvements.join(", ")}.`;
    }
    ...
    // Try to get more context from Claude's response if available
    let claudeContext = "";
    try {
      ...
      if (
        ...
        // Extract Claude's response for context
            ...
            for (const content of output.message.content) {
              if (content.type === "text" && content.text) {
                ...
                if (sentences.length > 0) {
                  claudeContext = sentences[0].trim() + ".";
                  break;
                }
              }
            }
            if (claudeContext) break;
          }
        }
      }
    } catch (error) {
    ...
    const mrDescription = `## ${purposeDescription}

${claudeContext ? `${claudeContext}\n\n` : ""}${changesDetail} ${filesContext}

**Original request:** [${process.env.CLAUDE_RESOURCE_TYPE?.toUpperCase()} #${process.env.CLAUDE_RESOURCE_ID}](${resourceUrl})

---
🤖 *This merge request was automatically generated by Claude AI* /cc @${process.env.GITLAB_USER_LOGIN || "claude"}`;
    ...
    // Create a simplified description for push options (no newlines allowed)
    const simpleMrDescription = `${purposeDescription}. ${changesDetail} ${filesContext} - Generated by Claude AI from ${process.env.CLAUDE_RESOURCE_TYPE} #${process.env.CLAUDE_RESOURCE_ID}`;
    ...
    if (mrUrlMatch) {
      ...
      try {
        ...
        console.log("✅ Updated MR description with full details");
      } catch (descError) {
        console.error("Failed to update MR description:", descError);
        // Continue anyway - the MR was created successfully
      }
      ...
      if (prepareResult.commentId) {
        ...
        await scmProvider.createComment(
          `🎯 I've created a merge request with the changes: ${mrUrl}\n\nPlease review and merge if the changes look good.`,
        );
      }
    }
  } catch (error) {
